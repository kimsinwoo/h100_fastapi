...
export default function TrainingPage() {
  ...
  const handleAdd = useCallback(async () => {
    ...
    setMessage(null);
    try {
      await addTrainingItem(addFile, addCaption);
      setAddFile(null);
      setAddCaption("");
      await fetchItems();
  ...
  const handleSaveCaption = useCallback(
    async (id: string) => {
      try {
        await updateTrainingCaption(id, editCaption);
        setEditingId(null);
        await fetchItems();
      } catch (err) {
        setMessage({ type: "error", text: getErrorMessage(err) });
        ...
        <div className="space-y-6">
                  ...
                  <li key={item.id} className="flex flex-col gap-3 rounded-lg border border-gray-200 p-3 sm:flex-row sm:items-start">
                    ...
                    <div className="min-w-0 flex-1">
                      {editingId === item.id ? (
                        ...
                        <>
                          <p className="text-sm text-gray-700">{item.caption || "(캡션 없음)"}</p>
                          <button
                            type="button"
          ...
          <section className="rounded-xl bg-white p-6 shadow">
            ...
            <p className="mb-4 text-xs text-gray-500">
              데이터가 준비되면 학습을 시작할 수 있습니다. 서버에서 백그라운드로 실행되며 완료까지 시간이 걸릴 수 있습니다.
            </p>
            <button
              type="button"
